...
export const updateProperty = async (req: Request, res: Response) => {
  ...
  if (!hasData(id)) {
    ...
    if (hasData(propertyType)) {
      ...
      //   throw new Error("`propertyType` must be a valid enum member if provided.");
      // }
    ...
    if (hasData(trade_name)) {
      existing.trade_name = trade_name;
    }
...
// ─── Update User if provided ────────────────────────────────────────
  ...
  // ─── Update description ─────────────────────────────────────────────────
    ...
    if (hasData(totalUnits)) {
      if (isNaN(Number(totalUnits))) {
        throw new Error("`totalUnits` must be a valid number if provided.");
      }
      existing.totalUnits = Number(totalUnits);
    }
