...
export const editPropertyUserVariant = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (hasData(total_units_available)) existing.total_units_available = Number(total_units_available);
    if (hasData(min_rate)) existing.min_rate = Number(min_rate);
    if (hasData(child_count)) existing.min_rate = Number(child_count);
